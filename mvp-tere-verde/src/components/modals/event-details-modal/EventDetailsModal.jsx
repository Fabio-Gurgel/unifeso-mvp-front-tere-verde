...
export function EventDetailsModal({ event, isOpen, onClose, onViewPark }) {
  ...
  const getNomeParqueMapeado = (eventName) => {
    ...
    if (
      nome.includes("observação de aves") ||
      nome.includes("observacao de aves")
    ) {
  ...
  if (!park) {
    const idDoParque =
      event.parque_id || (event.parque_ids && event.parque_ids[0]);
            ...
            <ul className={styles.metricsGrid}>
              ...
              <li>
                <Card className={styles.metricBlock}>
                  ...
                  <span className={styles.metricValue}>
                    {event.cronograma?.horario_inicio} -{" "}
                    {event.cronograma?.horario_fim}
                  </span>
                </Card>
              </li>
              <li>
                <Card className={styles.metricBlock}>
                  ...
                  <span className={styles.metricValue}>
                    {event.valor_entrada > 0
                      ? `R$ ${event.valor_entrada.toFixed(2)}`
                      : "Gratuito"}
                  </span>
                </Card>
              </li>
              <li>
                <Card className={styles.metricBlock}>
                  ...
                  <span
                    className={`${styles.metricValue} ${getVagasClass(event.capacidade?.vagas_disponiveis)}`}
                  >
            ...
            {event.capacidade?.exige_inscricao && (
              <div className={styles.inscriptionNotice}>
                ...
                <div>
                  ...
                  <p>
                    Este evento requer inscrição prévia para garantir sua vaga.
                  </p>
                </div>
              </div>
            )}
